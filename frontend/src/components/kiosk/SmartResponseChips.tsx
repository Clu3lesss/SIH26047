      ...
      <div className="flex flex-wrap gap-2">
        {chips.map((chip) => (
          <button
            ...
            className={cn(
              'px-3.5 py-2 rounded-lg border text-sm font-medium touch-target transition-all duration-150 text-left',
              selected === chip.value
                ? 'bg-teal-600 text-white border-teal-600 scale-95 shadow-sm'
                : 'bg-white text-slate-700 border-slate-200 hover:border-teal-400 hover:bg-teal-50 hover:text-teal-800 active:scale-95 disabled:opacity-50'
            )}
          >
            {chip.label}
          </button>
